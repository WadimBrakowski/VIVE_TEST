...
using UnityEngine;
namespace Wave.OpenXR.Samples.Ray
{
    public class WaveRay : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            StartCoroutine(Hiding_or_Showing_Ray());
        }
        ...
        public void Update_Ray()
        {
            ...
            if (HittingTarget != null)
            {
                ...
                {
                    Update_RayPoints();
                    foreach (Vector3 _Pos in Points)
                    {
                        //GObjPoints.Add(Instantiate(Ball, _Pos, Ball.transform.rotation));
                    }
                    ...
                    //RM.AddMesh_TypeTwo(RM.Get_TubeIndex(Start, RayPoints[i + 1].Pos, RayPoints[i].Radius, RayPoints[i].Radius, RayPoints[i].Dir).ToArray());
                    int _Temp = 0;
                    ...
                    //Debug.Log(_Temp);
                    //RM.AddMesh_TypeOne(RM.Get_CornIndex(RayPoints[RayPoints.Count - 2].Pos, RayPoints[RayPoints.Count - 1].Pos, RayPoints[RayPoints.Count - 2].Radius, RayPoints[RayPoints.Count - 1].Dir).ToArray());
                    RM.UpdateMesh();
                }
                else
                {
            ...
            {
                //RM.AddMesh_TypeOne(RM.Get_CircleIndex(Vector3.zero, RayPose.forward).ToArray());
                //RM.AddMesh_TypeOne(RM.Get_CircleIndex(Vector3.right * 3, RayPose.forward).ToArray());
                //RM.AddMesh_TypeOne(RM.Get_CircleIndex(Vector3.left * 3, RayPose.forward).ToArray());
                //RM.AddMesh_TypeOne(RM.Get_CornIndex(Vector3.right * 3, Vector3.right * 3 + Vector3.forward * 6, RayRadius, Vector3.forward).ToArray());
                //RM.AddMesh_TypeTwo(RM.Get_TubeIndex(Vector3.right * 3, Vector3.right * 3 + Vector3.forward * 10, 2, 2, Vector3.zero).ToArray());
                //RM.AddMesh_TypeTwo(RM.Get_TubeIndex(Vector3.left * 3, Vector3.left * 3 + Vector3.forward * 10, 2, 2, Vector3.zero).ToArray());
                //RM.AddMesh_TypeTwo(RM.Get_TubeIndex(Vector3.left * 6, Vector3.left * 6 + Vector3.forward * 10, 2, 2, Vector3.zero).ToArray());
        ...
        List<Vector3> _Get_Points()
        {
            ...
            for (int i = 0; i <= 20; i++)
            {
                ...
                float _Angle = (Start_Angle + i * Scale);
                float _AngleR = _Angle * Mathf.PI / 180;
                _Scaler = _ButtonLength / Mathf.Cos(_AngleR);
                _CurrentPoint = _CurrentPoint + (Quaternion.AngleAxis(_Angle, Normal) * -Dir_to_MeetingPoint) * _Scaler;
                //Debug.Log("Cos: " + (_Angle < 90 ? Mathf.Cos(_Angle) : -Mathf.Cos(_Angle))  + ", Scaler: " + _Scaler);
                _Points.Add(_CurrentPoint);
            }
            */
        ...
        List<Vector3> Get_Points()
        {
            ...
            while (_FirstTime || Vector3.Angle((_Points[_Points.Count - 1] - StartPoint), EndPoint - StartPoint) > 90)
            {
                ...
                /*
                float _Sacler2 = Vector3.Distance(Meeting_Point, Point) / (Vector2.Distance(_Points[_Points.Count - 1], StartPoint) + Vector3.Distance(Meeting_Point, Point));
                float _TotalTan = 0;
                for (int i = 1; i < _Points.Count; i++)
                {
                    _TotalTan += _PointTans[i];
                }
                for (int i = 1; i < _Points.Count; i++)
                {
                    //_Points[i] += (StartPoint - Meeting_Point) * (_PointTans[i] / _TotalTan);
                }
                //_Points.RemoveAt(0);
                if (_Points.Count > 0)
                {
        ...
        void SetUp()
        {
            ...
            //Debug.Log("Dis: " + Get_Dis_between_Points_on_ClipPlane(Point, RayPose.position, RayPose.position));
            //Debug.Log("Range: " + CastRange);
            //Debug.Log("PointA: " + Point + ",PointB: " + RayPose.position);
        ...
        {
            //Debug.Log(ProjectPointOnPlane(Camera.main.transform.forward, _PlanePoint, _PointA) + ", " + ProjectPointOnPlane(Camera.main.transform.forward, _PlanePoint, _PointB));
            //return Vector3.Distance(Vector3.ProjectOnPlane(_PointA, Camera.main.transform.forward) + Vector3.Dot(_PointA, Camera.main.transform.forward) * Camera.main.transform.forward, Vector3.ProjectOnPlane(_PointB, Camera.main.transform.forward) + Vector3.Dot(_PointB, Camera.main.transform.forward) * Camera.main.transform.forward);
        ...
        IEnumerator Hiding_or_Showing_Ray()
        {
            int _NotChangingFrameCount = 0;
            Vector3 _LastPos = Vector3.zero;
            Quaternion _LastRotation = Quaternion.identity;
            while (true)
            {
                if(_LastPos != RayPose.localPosition || _LastRotation != RayPose.localRotation)
                {
                    _NotChangingFrameCount = 0;
                }
                else
                {
                    _NotChangingFrameCount++;
                }
                if(_NotChangingFrameCount > 2)
                {
                    RayMesh.SetActive(false);
                }
                else
                {
                    RayMesh.SetActive(true);
                }
                _LastPos = RayPose.localPosition;
                _LastRotation = RayPose.localRotation;
